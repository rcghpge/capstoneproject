...
# -*- coding: utf-8 -*-
"""
IMR XGBoost Regression Model 2 — XGBoost 3.x
...
Outputs (in --outdir):
  ...
  - residuals_hist.png
  - predictions_with_ids.csv (if --save-predictions-with-ids)
"""
...
def parse_args():
    ...
    p.add_argument("--tree-method", default=None, help=("XGBoost tree construction algorithm. Options: 'hist', 'gpu_hist', 'exact', 'approx'"))
    ...
    p.add_argument("--save-predictions-with-ids", action="store_true",
                   help="Write predictions_with_ids.csv (IDs + Actual_IMR + Predicted_IMR).")
    return p.parse_args()
...
def write_regression_report_txt(report_path: Path, args, X, y, X_train, X_test,
    ...
    lines.append("=" * 80)
    lines.append(f"Timestamp: {_utc_now_iso_z()}")
...
def maybe_write_classification_report_txt(outdir: Path, threshold: float, y_true_cont, y_pred_cont):
    y_true_cls = (np.asarray(y_true_cont) >= threshold).astype(int)
    y_pred_cls = (np.asarray(y_pred_cont) >= threshold).astype(int)

    labels = [0, 1]
    target_names = ["Low", "High"]

    n0_true = int((y_true_cls == 0).sum())
    n1_true = int((y_true_cls == 1).sum())
    n0_pred = int((y_pred_cls == 0).sum())
    n1_pred = int((y_pred_cls == 1).sum())
    ...
    lines.append("")
    lines.append("Class counts (true):   Low= %d | High= %d" % (n0_true, n1_true))
    lines.append("Class counts (pred):   Low= %d | High= %d" % (n0_pred, n1_pred))
    if n0_true == 0 or n1_true == 0:
        lines.append("")
        lines.append("[note] Only one class present in true labels at this threshold; metrics for the absent class are set with zero_division=0.")
    lines.append("")
    lines.append("Confusion Matrix [rows: true, cols: pred] (Low=0, High=1)")
    lines.append("-" * 80)
...
def write_portable_txt_summary(outdir: Path, args, y_test, y_pred, metrics,
    ...
    if getattr(args, "save_predictions_with_ids", False):
        lines.append("predictions_with_ids.csv")
    lines.append("")
...
def main():
    ...
    tree_method = args.tree_method or ("gpu_hist" if args.gpu else "hist")
    xgb = XGBRegressor(
        ...
        objective="reg:squarederror", # xgb regression options: squarederror, pseudohubererror, squaredlogerror, quantileerorr
        n_jobs=args.n_jobs,
        random_state=args.random_state,
    ...
    if use_early_stop:
        ...
        if supports_callbacks:
            ...
            if args.progress and tqdm is not None:
                _ = TQDMCallback(total_rounds=args.n_estimators, enable=False)
            pipe = Pipeline([("pre", pre), ("reg", xgb)])
    ...
    if args.save_predictions_with_ids and args.id_cols:
        id_cols_present = [c for c in args.id_cols if c in df.columns]
        ids_test = df.loc[y_test.index, id_cols_present] if id_cols_present else pd.DataFrame(index=y_test.index)
        pred_out = pd.concat([
            ids_test.reset_index(drop=True),
            pd.Series(y_test.values, name="Actual_IMR"),
            pd.Series(y_pred, name="Predicted_IMR"),
        ], axis=1)
        pred_out.to_csv(outdir / "predictions_with_ids.csv", index=False, encoding="utf-8")
